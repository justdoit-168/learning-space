export default {
  ...
  methods: {
    extendMarkdown(md) {
      ...
      md.renderer.rules.image = (tokens, idx, options, env, self) => {
        console.log('md.renderer.rules.image---tokens--- ',tokens);
        const token = tokens[idx];
        console.log("3");
    ...
    // console.log(' md.renderer.rules.text-', tokens[idx],) ;
    //    let  content = tokens[idx]['content']
    //    // content: "![image-20211124220454855](./30C强制类型转换 .assets/image-20211124220454855.png)"  此类图片无法识别
    //   // return   md.renderer.rules.text(tokens, idx, options, env)
 ...
 this.img_prefix = '.'+ relative_path  + (tab_obj['modules_obj']['relative_folder'] ||'')
      ...
      console.log(' this.img_prefix ---', this.img_prefix );
      // console.log('this. MainComponent',this. MainComponent);
    }
  },
};
